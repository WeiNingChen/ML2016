...
def grad_cross_entropy(dataset, w, lmbda):
  '''
  ...
  return g / len(data_y) + 0.5*lmbda*w
...
def ERM_solver(dataset, loss, grad_loss, model_init = 0,  eta = 0.1, it = 60000, lmbda = 0.1): 
  [data_X, data_y]=dataset
  if(str(model_init) == '0') :
...
class lr_model:
  ...
  '''
  def __init__(self, loss = cross_entropy, grad_loss = grad_cross_entropy, model_init = 0, eta = 0.1, it = 10000, lmbda = 0.1):
    self.loss = loss
